...
This script will:
  ...
  5. Create the BashBard launcher
USAGE
}
...
if [[ -f "$REQ_FILE" ]]; then
  info "Installing dependencies from requirements.txt..."
  {
    if [[ "$MODE" == "system" ]]; then
      ...
      "$PY" -m pip install "${PIP_ARGS[@]}" -r "$REQ_FILE" --no-warn-script-location
    fi
  } || warn "Some dependencies may have version conflicts; continuing..."
else
...
if [[ "$MODE" != "system" ]]; then
  if [[ ":$PATH:" != *":$HOME/.local/bin:"* ]]; then
    ...
    success "Added ~/.local/bin to your PATH (reload terminal to apply)"
  fi
fi
